...
ps(void){
  ...
  char* state, *cmd;
  uint64 pid, ppid, ctime, stime, etime, sz;
  // printf("\n");
  for(p = proc; p < &proc[NPROC]; p++){
    acquire(&p->lock);
    if(p->state==UNUSED){
...
// Wait for a child process with given pid to exit and return its pid.
// Return -1 if this process has no child with the asked pid.
int
waitpid(int req_pid, uint64 addr)
{
  struct proc *np;
  int havekids, pid;
  struct proc *p = myproc();
